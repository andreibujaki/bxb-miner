...
#include "kernel\wolf-aes.cl"
#include "kernel\wolf-skein.cl"
#include "kernel\jh.cl"
#include "kernel\blake256.cl"
#include "kernel\groestl256.cl"
...
{ \
	...
	{ \
		...
		\
		\
		long prev = *((__global long*)(Scratchpad + (IDX((as_ulong(c.s01) & 0x1FFFF0) >> 4)))); \
		long cur = *((__global long*)(Scratchpad + (IDX((as_ulong(c.s01) & 0x1FFFF0) >> 4))) + 1); \ 
		*((__global long*)(Scratchpad + (IDX((as_ulong(c.s01) & 0x1FFFF0) >> 4))) + 1) = prev ^ cur; \
		\
		a ^= tmp; \
		\
		b_x = c; \
...
__kernel void search6(__global ulong *states, __global uint *BranchBuf, __global uint *output, ulong Target, ulong Threads)
{
...
__kernel void search5(__global ulong *states, __global uint *BranchBuf, __global uint *output, ulong Target, ulong Threads)
{
...
__kernel void search3(__global ulong *states, __global uint *BranchBuf, __global uint *output, ulong Target, ulong Threads)
{
	const uint idx = get_global_id(0) - get_global_offset(0);
...
__kernel void search4(__global ulong *states, __global uint *BranchBuf, __global uint *output, ulong Target, ulong Threads)
{
	const uint idx = get_global_id(0) - get_global_offset(0);

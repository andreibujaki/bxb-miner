...
{ \
	...
	{ \
		uint4 c; \
		\
		c = Scratchpad[IDX((as_ulong(a.s01) & 0xFFFF0) >> 4)]; \
		c = AES_Round(AES0, AES1, AES2, AES3, c, a); \
		\
		b_x ^= c; \
		VARIANT##VAR##_1(b_x); \
		\
		Scratchpad[IDX((as_ulong(a.s01) & 0xFFFF0) >> 4)] = b_x; \
		\
		uint4 tmp; \
		tmp = Scratchpad[IDX((as_ulong(c.s01) & 0xFFFF0) >> 4)]; \
		\
		...
		Scratchpad[IDX((as_ulong(c.s01) & 0xFFFF0) >> 4)] = a; \
		VARIANT##VAR##_2(a.s23); \
		\
		a ^= tmp; \
		\
